...
class AVLTree(object):
	...
	def searchWord(self, wordNode):
		"""if self.__node != None:
			print("atual: {}".format(self.__node.getWord()))"""
		if self.__node == None:
			print("{} 0".format(wordNode.getWord()))

		elif self.__node.getHashcode() == wordNode.getHashcode():
			if self.__node.getWord() == wordNode.getWord():
				print("{} {}".format(self.__node.getWord(), self.getHeight() ), end = "")
				print(" " + ' '.join(map(str, self.__node.getOcurrence())))
			...
			if line == "$CONSULTAS":
				...
				while True:
					try:
						...
						w = wordNode(line, 0)
						w.setHashcode()
						#print("searching: {}: {}".format(w.getWord(), w.getHashcode()))
						avl.searchWord(w)
					except EOFError:
						break
				break
				...
				for word in line.split():
					w = wordNode(word, i)
					w.setHashcode()
					#print("{}: {}".format(w.getWord(), w.getHashcode()))
					avl.insert(w)
